...
// This grabs the Vercel variable if it exists, otherwise it safely falls back to your live Render link!
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const profileRes = await fetch(`${API_URL}/profile/${userEmail}`);
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      // Updated to use API_URL
      const response = await fetch(`${API_URL}/evaluations/${id}`, { method: "DELETE" });
